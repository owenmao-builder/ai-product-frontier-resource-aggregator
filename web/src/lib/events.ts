...
export function eventLabel(item:NewsItem): string {
  ...
  if (event.coverage) return event.coverage.sourceCount >= 2 ? `${event.coverage.sourceCount} 家集中报道` : `${event.articleCount} 篇合并`
  return event.mediaCount >= 2 ? `${event.mediaCount} 家集中报道` : `${event.articleCount} 篇合并`
}
