...
export function SourceModal({ isOpen, onClose, siteStats, sourceCount, windowHours, directSources = [] }: SourceModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center" role="dialog" aria-modal="true" aria-labelledby="sources-title">
      <div 
        className="absolute inset-0 z-0 bg-black/50 backdrop-blur-sm"
        onClick={onClose}
      />
      <div className="relative z-10 bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-2xl w-full mx-4 max-h-[85vh] overflow-hidden">
        <div className="sticky top-0 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-6 py-4 flex items-center justify-between z-10">
          <div>
            <h2 id="sources-title" className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
              ...
              return <div key={feed.id} className="mb-3 rounded-xl border border-emerald-200 dark:border-emerald-800 bg-emerald-50/50 dark:bg-emerald-900/10 p-4">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className={`text-xs ${status?.error ? 'text-amber-600 dark:text-amber-400' : 'text-emerald-700 dark:text-emerald-400'}`}>
                    {status?.error ? '更新失败' : status?.fetchedAt ? '已连接' : '已配置'}
                  </span>
                </div>
