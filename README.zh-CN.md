...
- **直接给关注分：** 按模型升级、架构变更、速度提升、成本下降、热度和关注匹配立即评分，无需 API Key 或先完成证据核验。
...
菜单栏和内置看板按同一事件合并多篇报道；高亮“多家媒体关注”，2/3/4/5 家及以上分别加 0.3/0.5/0.7/1.0 分，10 分封顶。同媒体转载只计一次，官方、作者和聚合平台单独标记。展开可看核心分析、各家关注点与原文，区分标题归纳和正文摘录。每轮仅补充今天最多 3 个事件、9 篇公开正文及 6 段翻译，其中保留一个名额给不足 7 分的技术新闻；无需模型 API。来源筛选能检索事件内各篇文章；读过的事件增加转载仍保持已读。不同版本与故障、传闻、后续价格变化保持分开。
