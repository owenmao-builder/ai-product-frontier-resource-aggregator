...
菜单栏和内置看板按同一事件合并多篇报道；高亮“多方集中关注”及对应最低分。展开先列出计入热度的媒体与作者/社区，再看核心分析、各家关注点与原文，区分标题归纳和正文摘录。每轮仅补充今天最多 3 个事件、9 篇公开正文及 6 段翻译，其中保留一个名额给不足 7 分的技术新闻；无需模型 API。来源筛选能检索事件内各篇文章；读过的事件增加转载仍保持已读。不同版本与故障、传闻、后续价格变化保持分开。
