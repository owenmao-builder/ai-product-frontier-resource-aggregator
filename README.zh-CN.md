...
- **集中报道优先：** 多家媒体和资讯作者在短时间内共同关注，即可独立进入高分重点；不用等技术细节齐全。再看模型、架构等具体变化，无需 API Key 或先完成证据核验。
...
- **近期热门产品：** 从已采集的 X / Twitter 作者、媒体与社区讨论自动发现具名产品，同时接入 Hugging Face 模型趋势榜。显示独立作者数、集中报道来源、HF 榜位、GitHub 周增星数或 HN 讨论；较早发布的大厂产品也可凭近期热度入选。
...
**先看同一事件被多少不同来源集中报道。** 最近 48 小时内，2/3/5/8/12 个独立来源分别对应至少 7/8/8.5/9/9.5 分；最终分取该最低分与内容分的较高值。即使没有速度、成本和架构数据，集中报道也能独立成为重点。媒体和作者/社区数量分别标出；同源多篇、相同 URL/标题、纯转载、官方自述和聚合入口不重复扩大报道面。

默认按**关注分**降序排序。模型升级、架构变更的基础权重各 30，速度提升、成本下降及热度各 25、关注匹配 15；只计适用维度，按实际权重归一到 0–10 分。速度和成本按来源宣称的提升幅度分档，区间取较小倍数；正文和对应官方发布说明也参与。新名字只要符合模型、架构或效率兴趣，匹配为 3/5，名单内为 5/5。纯产品消息使用“产品/技术进展”；模型发布不必改变架构，缺少热度按 2.5/5 中性估值。普通品牌提及、传闻和旧模型接入不算新模型升级。

例如 Jev：新决策模型、架构变化、官网宣称的数量级速度与成本改善，再加近期 HN 热度，关注分为 9.2。展开显示官方中文重点、适用场景和来源；官方补充材料与各家媒体的观点分别标注。
...
每轮新闻采集同时发现具体模型、架构、工具和独立产品，不再依赖预设大厂型号。RSS 保留完整帖文的纯文本与原始项目链接，按实际主体归属讨论：Kev 的报道不会借给 Jev 或 Qwen 底座，Muse、Muse Realtime Avatar 与不同模型版本分别记录。社区条目即使缺少官方首发日也能凭热度进入热门，展开可看原始讨论；“大厂上新”仍只按官方首发日筛选最近 7 个北京时间自然日。模型仓库创建时间、抓取日和转载日都不能冒充正式发布日期。

自动发现无需模型 API Key。X 的覆盖来自已配置的公开订阅及采集来源，不能保证全 X 完整；仅统计实际取得的独立作者，不编造点赞、转发或全站排名。内置编辑资料补充 Muse、Kev、Laya 等产品的官方说明，后续新名字通过新闻与 HF 趋势自动发现；社区资料与已核对的官方资料明确区分。产品同步不重新标注历史新闻。

近期热门满足任一信号：近 7 天至少 2 个独立媒体/作者/社区讨论（其中 X 作者单独标注）、当天新闻事件集中报道、官方 Hugging Face 趋势榜、GitHub 周榜本周新增至少 500 星，或近 7 天 HN 至少 100 赞。官方账号自述、纯转发和同一来源多篇不扩大外部来源数。X/社区讨论按真实发表时间保留 7 天；HF/GitHub/HN 热度核对超过 24 小时失效，HF 正常每小时更新。入榜表示近期关注，不代表性能验证。
...
菜单栏和内置看板按同一事件合并多篇报道；高亮“X 家集中报道”。展开先列出报道来源，再看核心分析、各家关注点与原文，区分标题归纳和正文摘录。每轮仅补充今天最多 3 个事件、9 篇公开正文及 6 段翻译，其中保留一个名额给不足 7 分的技术新闻；无需模型 API。来源筛选能检索事件内各篇文章；读过的事件增加转载仍保持已读。不同版本与故障、传闻、后续价格变化保持分开。没有型号的“IPO 前考虑发布新模型”消息按公司、动作与事件语境合并中英文改写；IPO 延期、营收、正式发布和否认保持分开。原始媒体的短链归回同一来源。
